...
def make_image(plot_variables):
    ...
    df[plot_variables["var_column"]] = pd.to_numeric(
        df[plot_variables["var_column"]], errors="coerce"
    )
    df[plot_variables["time_column"]] = pd.to_datetime(
        df[plot_variables["time_column"]]
    )
    fig = Figure(figsize=(10, 5))
    ax = fig.subplots()
    ax.plot(
        df[plot_variables["time_column"]],
        df[plot_variables["var_column"]],
        color=plot_variables["color"],
    )

...
class decomposition_residuals:
    ...
    def calculate_residuals_detrend(self, df):
        ...
        for i in range(len(df[self.variables["variable"]]) - 1):
            value = (
                df[self.variables["variable"]][i + 1]
                - df[self.variables["variable"]][i]
            )
        ...
        residual_df = pd.DataFrame(
            {time_column: date_list, self.variables["variable"]: detrend_list}
        )
    ...
    def add_residuals(self):
        df = pd.read_csv(
            current_app.config["UPLOAD_FOLDER"] + self.variables["dataset"]
        )
